...
# Robust path to data
# Go up 3 levels: src/lynchstreamlit/Transformer.py -> src/lynchstreamlit -> src -> root
...
try:
    ...
    for index, row in file.iterrows():
        questions.append(preprocess_sentence(str(row["question"])))
        answers.append(preprocess_sentence(str(row["answer"])))

    tokenizer = tfds.deprecated.text.SubwordTextEncoder.build_from_corpus(
        questions + answers, target_vocab_size=2**13
    )
except Exception as e:
    print(f"Warning: Could not load tokenizer data from {data_path}. {e}")
    # Fallback or dummy tokenizer if file missing (e.g. during CI)
    tokenizer = None
...
class PositionalEncoding(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        if isinstance(inputs, tf.SparseTensor) or getattr(inputs, "sparse", False):
